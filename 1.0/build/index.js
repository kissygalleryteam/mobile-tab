 ...
 * @module mobile-tab
 **/
KISSY.add('gallery/mobile-tab/1.0/index',function (S, Node,Base) {
	...
	*/
	...
	function MobileTab( tab , options){ 

		this.tab = tab;
		this.tabs = this.tab.all("li");//.slice(0, -1);
		this.silder = this.tab.all(".J_Slider");
		this.tabIn = this.tab.all(".J_In");
	...
	MobileTab.prototype = {
		...
		initOffsets:function(){
			...
			for(var i=0; i<lis.length; i++){
				...
				lis.item(i).attr("data-tabnavindex", i);
		...
		render:function(){
			...
			if( this.isHaveScroll() ){
				...
				this.tabs.item(this.tabs.length-1).css("margin-right", gap+"px");
		...
		updateCurrentIndex:function( index ){
			...
			this.tabs.item(this.currentIndex).removeClass("on");
			this.tabs.item(index).addClass("on");
			this.currentIndex = index;
		    this.fire("onChange", [index, currentIndex, this.tabs.item(index)]);
		},
		go:function( index ){
